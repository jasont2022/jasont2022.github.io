...
const colorArray = [
  { code: 'CIS 545', name: 'Data Science', color: '#F9D976' },
...
// a variable to indicate if the page is on the study session display
let isOnStudySession = true

// get the background container
const container = document.querySelector(".background")

// create the legend div for future use
const legend = document.createElement("div")

// display the study sessions
...
buttonDiv.addEventListener("click", () => {
  if (isOnStudySession) {
    ...
    displayLegend()
    buttonDiv.innerHTML = 'Return'
    isOnStudySession = false
  } else {
    container.removeChild(legend)
    container.appendChild(dates)
    buttonDiv.innerHTML = 'Learn More!'
    isOnStudySession = true
  }
})
...
function displayStudySessions() {
  let i = 0
  const datesDiv = container.appendChild(dates)
  studySessions.forEach(studySession => {
    const newItem = document.createElement("div")
    const dateDiv = datesDiv.appendChild(newItem)
    i++;
